...
#   12. Incorrect key length error message is removed in DES/TDES
#   13. "Use output as the key" function is added.
#   14. Correct all fonts !!!
#   15. Added a digital clock for 120PIN entry testing !!!
...
'''
root = tk.Tk()
root.title('CryptoBox v00.09.11.x20170121')
...
algo_tab.add(frame_8_ABT, text='About CryptoBox\n')
algo_tab.pack()
...
realtime = tk.Label(root, text="", font=("Fixedsys", 20))
...
class CryptoBox(tk.Tk):
    ...
    def execution_TDES(self):
        ...
        self.key_ck_value_TDES.insert(1.0, key_len_check)
        ...
        if selection_Algo == 0:  # non DES/TDES algo
            ...
            if selection_EorD == 0:
            ...
            elif selection_EorD == 1 and key_len == 16 or key_len == 24:  # Enc operation
                #   'mode' Judgment - single DES does't need a mode selection
                #   ECB as a default mode
                # mode = DES.MODE_ECB
                ...
                self.output_textbox_TDES.delete(1.0, END)
                self.output_textbox_TDES.insert(1.0, h_out_data_e)
            elif selection_EorD == 2 and key_len == 16 or key_len == 24:  # Dec operation
                #   'mode' Judgment - single DES does't need a mode selection
                #   ECB as a default mode
                # mode = DES.MODE_ECB
                obj = DES3.new(hkey_xDES, mode, hiv_xDES)
    ...
    def RSA_key_pair_gen_func(self):
        ...
        #
        bin_pvtKey = private.exportKey('DER')
        bin_pubKey = private.publickey().exportKey('DER')


        priKeyObj = RSA.importKey(bin_pvtKey)
        pubKeyObj = RSA.importKey(bin_pubKey)

        msg = "plaintext data input"

        emsg = pubKeyObj.encrypt(msg, 0)[0]
        '''
        with open("emsg.bin", "wb") as msg_file:
            #text = 'hello world'
            msg_file.write(emsg.encode('utf-8'))
        '''
        dmsg = priKeyObj.decrypt(emsg)

        print "\nbin_pvtKey:", bin_pvtKey
        print "bin_pubKey:", bin_pubKey
        #   binascii.b2a_hex() is used for converting sth to hex string!!!!!
        print "\nbin_pvtKey:", binascii.b2a_hex(bin_pvtKey)
        print "bin_pubKey:", binascii.b2a_hex(bin_pubKey)
        print "\nmsg:", msg
        print "\nemsg:", emsg
        print "emsg:", binascii.b2a_hex(emsg)
        print "\ndmsg:", dmsg
        #
        print '\npri:', private.e, private.d, private.p, private.q, private.n
        print '\npub:', public
        ...
        '''
        substrate = pem.readPemFromFile(open('pub.pem'))
        cert = decoder.decode(substrate, asn1Spec=rfc2459.Certificate())[0]
        print "LOOK AT HERE !!!", (cert.prettyPrint())
        '''
    ...
    def execution_RSA_enc(self):
        ...
        rsa_p_data = self.rsa_data_in.get("1.0", END).encode('hex')
        print 'rsa_p_data:', rsa_p_data
        with open('pub.pem', 'r') as pub_key_file:
            #
            '''
            lines = pub_key_file.replace(' ', '').split()
            der = a2b_base64(''.join(pub_key_file[1:-1]))
            cert = DerSequence()
            cert.decode(der)
            tbsCertificate = DerSequence()
            tbsCertificate.decode(cert[0])
            subjectPublicKeyInfo = tbsCertificate[6]
            return_value = RSA.importKey(subjectPublicKeyInfo)
            ...
            f = open("output_Enc.bin", "w")
            for i in range(len(enciphered_data)):
                f.write("%s" % enciphered_data)
            f.close()

    def execution_RSA_dec(self):
        rsa_c_data = self.rsa_data_in.get("1.0", END).encode('hex')
        print 'rsa_c_data:', rsa_c_data
        with open('pvt.pem', 'r') as pvt_key_file:
            ...
            f = open("output_Dec.bin", "w")
            for i in range(len(deciphered_data)):
                f.write("%s" % deciphered_data)
            f.close()
    ...
    def p120_start(self):
        ...
        self.start_tm.insert(1.0, "Started at XX-XX-XX XX:XX:XX")
        self.last_tm.insert(1.0, "Started at:         XX-XX-XX XX:XX:XX\n")
    def p120_go(self):
        #self.last_tm.delete(1.0, END)
        self.last_tm.insert(0.0, "#xxx PIN entry at:  XX-XX-XX XX:XX:XX\n")
    def p120_end(self):
        #self.last_tm.delete(1.0, END)
        self.last_tm.insert(0.0, "The last entry at:  XX-XX-XX XX:XX:XX\n")
        self.last_tm.insert(0.0, "-------   Elapsed timetable   -------\n")
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.algo_bar_TDES = tk.LabelFrame(frame_1_TDES, text="DES/TDES", font=("Fixedsys", 12, "bold"), padx=10, pady=10)
        ...
        self.operation_bar_TDES = tk.LabelFrame(frame_1_TDES, text="Operation", font=("Fixedsys", 12, "bold"), padx=10, pady=10)
        ...
        self.Mode_bar_TDES = tk.LabelFrame(frame_1_TDES, text="Modes", font=("Fixedsys", 12, "bold"), padx=10, pady=10)
        ...
        self.key_textbox_TDES = tk.Entry(frame_1_TDES, font = "Courier 10", width=47)
        ...
        self.key_ck_value_TDES = tk.Text(frame_1_TDES, font = "Courier 10", height=1, width=4)
        ...
        self.iv_textbox_TDES = tk.Entry(frame_1_TDES, font = "Courier 10", width=47)
        ...
        self.input_textbox_TDES = tk.Entry(frame_1_TDES, font = "Courier 10", width=47)
        ...
        self.output_textbox_TDES = tk.Text(frame_1_TDES, font = "Courier 10", height=8, width=47)
        ...
        self.go_button_TDES.grid(row=9, column=2, padx=5, pady=5, sticky=E)

        #   2.1 AES - Encryption or Decryption Selection Frame
        self.EncOrDec_bar_AES = tk.LabelFrame(frame_2_AES, text="Operation", font=("Fixedsys", 12, "bold"), padx=10, pady=10)
        ...
        self.Mode_bar_AES = tk.LabelFrame(frame_2_AES, text="Modes", font=("Fixedsys", 12, "bold"), padx=10, pady=10)
        ...
        self.key_textbox_AES = tk.Entry(frame_2_AES, font = "Courier 10", width=47)
        ...
        self.iv_textbox_AES = tk.Entry(frame_2_AES, font = "Courier 10", width=47)
        ...
        self.input_textbox_AES = tk.Entry(frame_2_AES, font = "Courier 10", width=47)
        ...
        self.output_textbox_AES = tk.Text(frame_2_AES, font = "Courier 10", height=8, width=47)
        ...
        self.go_button_AES.grid(row=9, column=2, padx=5, pady=5, sticky=E)

        #   3_1   RSA Gen.

        #   3_1.1 RSA Key Gen
        self.Key_Gen = tk.LabelFrame(frame_3_1_RSA, text="RSA Key Pair Generation (PEM)", font=("Fixedsys", 12, "bold"), padx=5, pady=5)
        self.Key_Gen.grid(row=1, column=2, rowspan=4, sticky=W)

        self.RSA_pri_key_lb = tk.Label(self.Key_Gen, text="Private    ")
        self.RSA_pri_key_lb.grid(row=2, column=1, padx=1, pady=3, sticky=W+N)

        self.pri_key_textbox = tk.Text(self.Key_Gen, font = "Courier 9", height=8, width=64)
        self.pri_key_textbox.grid(row=2, column=2, padx=3, pady=3, sticky=W)

        self.RSA_pub_key_lb = tk.Label(self.Key_Gen, text="Public     ")
        self.RSA_pub_key_lb.grid(row=3, column=1, padx=1, pady=3, sticky=W+N)

        self.pub_key_textbox = tk.Text(self.Key_Gen, font = "Courier 9", height=8, width=64)
        self.pub_key_textbox.grid(row=3, column=2, padx=3, pady=3, sticky=W)

        self.RSA_key_pair_gen_bot = tk.Button(self.Key_Gen, text="Gen RSA key pair", width=11, command=self.RSA_key_pair_gen_func)
        self.RSA_key_pair_gen_bot.grid(row=4, column=2, padx=3, pady=3, sticky=E)

        self.RSA_k_len_lb = tk.Label(self.Key_Gen, text="key len")
        self.RSA_k_len_lb.grid(row=4, column=1, padx=3, pady=4, sticky=W)
        self.RSA_k_len_En = tk.Entry(self.Key_Gen, width=5)
        self.RSA_k_len_En.grid(row=4, column=2, padx=3, pady=3, sticky=W)
        ...
        self.rsa_data_work = tk.Label(frame_3_2_RSA, text="\nNote: Private key and Public key must be stored in the pvt.pem and pub.pem files\n", font=("Fixedsys", 8))
        self.rsa_data_work.grid(row=0, column=2, columnspan=3, sticky=W)

        self.rsa_data_in_lb = tk.Label(frame_3_2_RSA, text="Data Input ")
        self.rsa_data_in_lb.grid(row=1, column=1, sticky=W+N)
        self.rsa_data_in = tk.Text(frame_3_2_RSA, font = "Courier 10", height=8, width=56)
        self.rsa_data_in.grid(row=1, column=2, padx=3, pady=3, sticky=W)

        self.rsa_data_out_lb = tk.Label(frame_3_2_RSA, text="Data Output")
        self.rsa_data_out_lb.grid(row=2, column=1, sticky=W+N)
        self.rsa_data_out = tk.Text(frame_3_2_RSA, font = "Courier 10", height=8, width=56)
        self.rsa_data_out.grid(row=2, column=2, padx=3, pady=3, sticky=W)


        #   3_2.1 RSA Enc/Dec
        self.key_pub_butt_RSA = tk.Button(frame_3_2_RSA, text="Encrypt data", width=11, command=self.execution_RSA_enc)
        self.key_pub_butt_RSA.grid(row=3, column=2, padx=3, pady=5, sticky=E)
        self.key_pri_butt_RSA = tk.Button(frame_3_2_RSA, text="Decrypt data", width=11, command=self.execution_RSA_dec)
        self.key_pri_butt_RSA.grid(row=4, column=2, padx=3, pady=5, sticky=E)
        '''
        ...
        self.HASH_frame = tk.LabelFrame(frame_4_HASH, text="Algorithms", font=("Fixedsys", 12, "bold"), padx=10, pady=10)
        ...
        self.hash_hmac_key_entry = tk.Entry(frame_4_HASH, font = "Courier 10", width=48)
        ...
        self.hash_input_entry = tk.Entry(frame_4_HASH, font = "Courier 10", width=48)
        ...
        self.hash_output_text = tk.Text(frame_4_HASH, font = "Courier 10", height=8, width=48)
        ...
        self.go_button_hash.grid(row=9, column=2, padx=5, pady=5, sticky=E)

        #   5   XOR
        self.XOR_frame = tk.LabelFrame(frame_5_XOR, text="XOR", font=("Fixedsys", 12, "bold"), padx=10, pady=10)
        ...
        self.xor_inputA_value = tk.Entry(self.XOR_frame, font = "Courier 10", width=47)
        self.xor_inputA_value.grid(row=2, column=2, columnspan=2, padx=5, pady=5, sticky=W)
        self.xor_inputB_value = tk.Entry(self.XOR_frame, font = "Courier 10", width=47)
        self.xor_inputB_value.grid(row=3, column=2, columnspan=2, padx=5, pady=5, sticky=W)
        self.xor_result_value = tk.Text(self.XOR_frame, font = "Courier 10", height=8, width=47)
        ...
        self.rng_bar_bar = tk.LabelFrame(frame_6_RNG, text="Random number", font=("Fixedsys", 12, "bold"), padx=20, pady=10)
        ...
        self.rng_8B_textbox = tk.Text(self.rng_bar_bar, font = "Courier 10", height=1, width=48)
        ...
        self.rng_32B_textbox = tk.Text(self.rng_bar_bar, font = "Courier 10", height=4, width=48)
        ...
        self.rng_88B_textbox = tk.Text(self.rng_bar_bar, font = "Courier 10", height=4, width=48)
        self.rng_88B_textbox.grid(row=9, column=1, padx=5, pady=5)


        #   7   120 PINs
        self.pin_120_bar = tk.LabelFrame(frame_7_120, text="Elapsed time of 120 online PIN entry test", font=("Fixedsys", 12, "bold"), padx=5, pady=5)
        self.pin_120_bar.grid(row=1, column=1, columnspan=4)

        self.start_lb = tk.Label(self.pin_120_bar, text="PIN entry started at:")
        self.start_lb.grid(row=1, column=1, sticky=E, padx=3, pady=3)
        self.start_tm = tk.Text(self.pin_120_bar, font=("Courier", 10), height=1, width=42)
        self.start_tm.grid(row=1, column=2, sticky=W, padx=3, pady=3)


        self.last_lb = tk.Label(self.pin_120_bar, text="Last PIN entered at:")
        self.last_lb.grid(row=2, column=1, sticky=E, padx=3, pady=3)
        self.last_tm = tk.Text(self.pin_120_bar, font=("Courier", 10), height=8, width=42)
        self.last_tm.grid(row=2, column=2, sticky=W, padx=3, pady=3)
        ...
        #   8   About CryptoBox
        self.abt_bar = tk.LabelFrame(frame_8_ABT, text=" --- CryptoBox --- ", font=("Fixedsys", 12, "bold"))
