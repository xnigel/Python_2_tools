...
#   17. RSA calculation is solved partially!!!!!!! hahahahahaha
...
'''
root = tk.Tk()
root.title('CryptoBox v00.09.15.x20170223')
root.geometry("530x470+20+20")    #("560x470+0+0") for Linux; ("530+470+20+20") for Windows
...
class CryptoBox(tk.Tk):
    ...
    def RSA_key_pair_gen_func(self):
        ...
        bin_pubKey = public.exportKey('DER')


        #priKeyObj = RSA.importKey(bin_pvtKey)
        #pubKeyObj = RSA.importKey(bin_pubKey)

        data_rsa = 0x241234ffffffffffL

        #e_data_rsa = pubKeyObj.encrypt(data_rsa, 0)[0]
        e_data_rsa_2=public.encrypt(data_rsa, 0)[0]
        ...
        #p_data_rsa = priKeyObj.decrypt(e_data_rsa)
        p_data_rsa_2 = private.decrypt(e_data_rsa_2)
        ...
        #print "pubKeyObj:\n", pubKeyObj
        print "\n#  0  -----------------------------------------------------------------------------"
        print "bin_pvtKey:\n", binascii.b2a_hex(bin_pvtKey)
        print "\nbin_pubKey:\n", binascii.b2a_hex(bin_pubKey)
        print "\n#  1  -----------------------------------------------------------------------------"
        #print "bin_pvtKey\nbinascii.b2a_hex(bin_pvtKey):", binascii.b2a_hex(bin_pvtKey)
        #print "bin_pubKey\nbinascii.b2a_hex(bin_pubKey):", binascii.b2a_hex(bin_pubKey)
        print "\n#  2  -----------------------------------------------------------------------------"
        #print "e_data_rsa:\n", hex(e_data_rsa)
        print "e_data_rsa_2:\n", hex(e_data_rsa_2)
        #print "p_data_rsa:\n", hex(p_data_rsa)
        ...
        temp_e = hex(private.e)
        temp_d = hex(private.d)
        temp_n = hex(private.n)
        ...
        self.RSAgen_output_exp_tx.insert(1.0, hex(private.e))
        self.RSAgen_output_pri_tx.delete(1.0, END)
        self.RSAgen_output_pri_tx.insert(1.0, hex(private.d))
        self.RSAgen_output_pub_tx.delete(1.0, END)
        self.RSAgen_output_pub_tx.insert(1.0, hex(private.n))

        self.rsa_key_exp_import_tx.delete(1.0, END)
        self.rsa_key_exp_import_tx.insert(1.0, hex(private.e).rstrip("L").lstrip("0x"))
        self.rsa_key_pri_import_tx.delete(1.0, END)
        self.rsa_key_pri_import_tx.insert(1.0, hex(private.d).rstrip("L").lstrip("0x"))
        self.rsa_key_pub_import_tx.delete(1.0, END)
        self.rsa_key_pub_import_tx.insert(1.0, hex(private.n).rstrip("L").lstrip("0x"))

        with open("RSA_e.hex", "w") as RSA_e:
            RSA_e.write(hex(private.e))
        with open("RSA_d.hex", "w") as RSA_d:
            RSA_d.write(hex(private.d))
        with open("RSA_n.hex", "w") as RSA_n:
            RSA_n.write(hex(private.n))


    def rsa_key_import_func(self):
        print "testing##"
        rsa_input_exp = self.rsa_key_exp_import_tx.get("1.0", END)    #"1.0", END
        rsa_input_pri = self.rsa_key_pri_import_tx.get("1.0", END)
        rsa_input_pub = self.rsa_key_pub_import_tx.get("1.0", END)

        # 'test_e', 'test_d', 'test_n' store data in decimal format!
        test_e = 0x10001L
        test_d = 
        test_n = 

        with open('RSA_e.hex', 'r') as RSA_e_temp:
            test_value_e = RSA_e_temp.read()
        with open('RSA_d.hex', 'r') as RSA_d_temp:
            test_value_d = RSA_d_temp.read()
        with open('RSA_n.hex', 'r') as RSA_n_temp:
            test_value_n = RSA_n_temp.read()
        print "\n#  4.5  -----------------------------------------------------------------------------"
        '''with open("RSA_e.hex", "r") as RSA_e_temp:
            #RSA_e_temp.read(hex(private.e))
        with open("RSA_d.hex", "r") as RSA_d_temp:
            #RSA_d_temp.write(hex(private.d))
        with open("RSA_n.hex", "r") as RSA_n_temp:
            #RSA_n_temp.write(hex(private.n))
        with open("RSA_d_test.hex", "w") as RSA_d_test:
            RSA_d_test.write(0x1234567890abcdef)
        '''
        print "\n#  4.6  -----------------------------------------------------------------------------"
        print "\ntest_d:\n", test_d
        ...
        pri_const = RSA.construct((test_n, test_e, test_d))
        pub_const = RSA.construct((test_n, test_e))

        print "\n#  4.7  -----------------------------------------------------------------------------"
        print "\npub_const:\n", pub_const

        #pri_const = RSA.construct((test_value_n, test_value_e, test_value_d))
        #pub_const = RSA.construct((test_value_n, test_value_e))

        data_rsa = 0x241235ffffffffffL
        e_data_rsa = pub_const.encrypt(data_rsa, 0)[0]
        p_data_rsa = pri_const.decrypt(e_data_rsa)

        '''
        exp = rsa_input_exp.decode('hex')
        pri = rsa_input_pri.decode('hex')
        pub = rsa_input_pub.decode('hex')
        '''

        #print exp.encode('hex'), pri.encode('hex'), pub.encode('hex')

        #print "this is rsa_input_exp:", binascii.a2b_hex(rsa_input_exp)

        #RSAObj = RSA.construct((rsa_input_pub, rsa_input_exp))
        #RSAObj = RSA.construct((rsa_input_pri, rsa_input_exp))
        #RSAObj = RSA.construct((rsa_input_pri, rsa_input_exp))
        '''
        print "test_n\n", hex(test_n)
        print "test_e\n", hex(test_e)
        ...
        #print "IMPORTED_exp! :\n", binascii.a2b_hex(rsa_input_exp)
        #print "IMPORTED_pri! :\n", binascii.a2b_hex(rsa_input_pri)
        #print "IMPORTED_pub! :\n", binascii.a2b_hex(rsa_input_pub)

        #print "\nRSAObj", RSAObj
        #print "\nRSAObj.n", RSAObj.n, "\nRSAObj.e", RSAObj.e, "\nRSAObj.n(hex)", hex(RSAObj.n), "\nRSAObj.e(hex)", hex(RSAObj.e)
        self.rsa_key_exp_import_tx.delete(1.0, END)
    ...
    def execution_RSA_enc(self):
        # 1. get input plaintext data
        # rsa_D_in = StringVar()
        rsa_D_in = self.rsa_data_in.get("1.0", END)
        rsa_D_in_enC = self.rsa_data_in.get("1.0", END).encode('utf-8')
        print 'rsa_D_in:', rsa_D_in
        print 'rsa_D_in_enC:', rsa_D_in_enC

        # 2. wriet the plaintext data to a file <input_en_data.bin>
        p_in = open ("input_en_data.bin", "wb")
        p_in.write(rsa_D_in_enC)
        p_in.close()

        with open('pvt.pem', 'r') as rsa_master_key: # replace rsa_master_key instead pub_key_file
            #

            f = open('pvt.pem','r')
            test_key = RSA.importKey(f.read())

            print "f", f

            #not sure this is correct or not!!!!!!!!
            encworking_pri_key = RSA.importKey(rsa_master_key.read())
            encworking_bin_pub_key = encworking_pri_key.publickey().exportKey('DER')
            encworking_Obj_pub_key = RSA.importKey(encworking_bin_pub_key)

            #K =

            print "encworking_pri_key:", encworking_pri_key
            print "encworking_bin_pub_key", encworking_bin_pub_key
            print "encworking_Obj_pub_key", encworking_Obj_pub_key
            print "encworking_pri_key.d(hex):", hex(encworking_pri_key.d)
            print "???????", hex(encworking_pri_key.n)
            #print "encworking_bin_pub_key(hex)", hex(encworking_bin_pub_key)
            #print "encworking_Obj_pub_key(hex)", hex(encworking_Obj_pub_key)



            rsa_D_out = encworking_Obj_pub_key.encrypt(rsa_D_in, 0)[0]

            print "\nrsa_D_out:", rsa_D_out, "rsa_D_out(hex)", hex(rsa_D_out)


            self.rsa_data_out.delete(1.0, END)
            self.rsa_data_out.insert(1.0, rsa_D_out)
            # write the output data to a file:
            f = open("output_Enc.bin", "w")
            for i in range(len(rsa_D_out)):
                f.write("%s" % rsa_D_out)
            f.close()

    def execution_RSA_dec(self):

        rsa_c_data = self.rsa_data_in.get("1.0", END)
        rsa_c_data_enC = self.rsa_data_in.get("1.0", END).encode('utf-8')
        print 'rsa_c_data:', rsa_c_data

        p_out = open ("input_de_data.bin", "wb")
        p_out.write(rsa_c_data_enC)
        p_out.close()
        ...
        with open('pvt.pem', 'r') as pvt_key_file:
            ...
            print "pvt_key: \n", pvt_key, "\n", pvt_key.e, "\n", pvt_key.n, "\n", pvt_key.p,"\n"
            #   XXX Issues are here !!
            with open("input_de_data.bin", "r") as rsa_temp_data:
                deciphered_data = pvt_key.decrypt(rsa_temp_data)
                deciphered_data_enC = pvt_key.decrypt(rsa_temp_data).encode('hex')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rsa_key_exp_import_lb.grid(row=2, column=1, padx=5, pady=5, sticky =W+N)
        self.rsa_key_exp_import_tx = tk.Text(self.rsa_key_import, font = "Courier 9", height=1, width=64)
        self.rsa_key_exp_import_tx.grid(row=2, column=2, padx=5, pady=5, sticky=W)
        self.rsa_key_pri_import_lb = tk.Label(self.rsa_key_import, text="Private")
        self.rsa_key_pri_import_lb.grid(row=3, column=1, padx=5, pady=5, sticky =W+N)
        self.rsa_key_pri_import_tx = tk.Text(self.rsa_key_import, font = "Courier 9", height=8, width=64)
        self.rsa_key_pri_import_tx.grid(row=3, column=2, padx=5, pady=5, sticky=W)
        self.rsa_key_pub_import_lb = tk.Label(self.rsa_key_import, text="Public ")
        self.rsa_key_pub_import_lb.grid(row=4, column=1, padx=5, pady=5, sticky =W+N)
        self.rsa_key_pub_import_tx = tk.Text(self.rsa_key_import, font = "Courier 9", height=8, width=64)
        self.rsa_key_pub_import_tx.grid(row=4, column=2, padx=5, pady=5, sticky=W)

        self.rsa_key_pub_import_bot = tk.Button(self.rsa_key_import, text="Import your RSA key pair", width=20, command=self.rsa_key_import_func)
        self.rsa_key_pub_import_bot.grid(row=5, column=2, padx=5, pady=5, sticky=N+E)
        ...
        self.rsa_note = tk.Label(self.rsa_data_work,
                                      text="\nNote:\nPrivate key and Public key must be\npre-generated or pre-stored in the\npvt.pem and pub.pem files\n",
                                      font=("Helvetica", 8))
