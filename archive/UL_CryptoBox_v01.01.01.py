...
'''
root = tk.Tk()
root.title('CryptoBox v01.01.01 20170513')
root.geometry("540x500+20+20")    #("560x480+0+0") for Linux; ("530+470+20+20") for Windows
...
class CryptoBox(tk.Tk):
    ...
    def execution_TDES(self):
        ...
        print "DES input data         :", h_in_data_xDES.encode('hex')
        if key_len<9:
        ...
        if selection_Algo == 0:  # non DES/TDES algo
            ...
            print "TDES mode is           :", mode, " | <1:ECB, 2:CBC, 3:CFB, 4:OFB>"
            #   'Enc/Dec' Judgment, and execute!!!
            if selection_EorD == 0:
                self.output_textbox_TDES.delete(1.0, END)
    ...
    def RSA_key_pair_gen_func(self):
        ...
        with open("RSA_pub.pem", "w") as pub_key:
            pub_key.write(public.exportKey())
        with open("RSA_pvt.pem", "w") as pvt_key:
