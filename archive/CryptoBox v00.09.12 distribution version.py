...
'''
root = tk.Tk()
root.title('CryptoBox v00.09.12.x20170123')
root.geometry("560x500+10+10")    #("600x450+10+10") is in Linux; ("700x560+10+10") is in Windows
...
class CryptoBox(tk.Tk):
    ...
    def execution_RSA_enc(self):
        ...
        rsa_p_data = self.rsa_data_in.get("1.0", END)
        rsa_p_data_enC = self.rsa_data_in.get("1.0", END).encode('utf-8')
        print 'rsa_p_data:', rsa_p_data
        print 'rsa_p_data_enC:', rsa_p_data_enC

        # 2. wriet the plaintext data to a file <input_en_data.bin>
        p_in = open ("input_en_data.bin", "wb")
        p_in.write(rsa_p_data_enC)
        p_in.close()
        with open('pub.pem', 'r') as pub_key_file:
            ...
            #   XXX Issues are here !!
            enciphered_data = pub_key.encrypt(rsa_p_data_enC, 0)[0].encode('hex')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.algo_bar_TDES = tk.LabelFrame(frame_1_TDES, text="DES/TDES", font=("Helvetica", 12, "bold"), padx=5, pady=5, bd=4)
        ...
        self.operation_bar_TDES = tk.LabelFrame(frame_1_TDES, text="Enc/Dec", font=("Helvetica", 12, "bold"), padx=5, pady=5, bd=4)
        ...
        self.Mode_bar_TDES = tk.LabelFrame(frame_1_TDES, text="Modes", font=("Helvetica", 12, "bold"), padx=5, pady=5, bd=4)
        ...
        self.key_textbox_TDES = tk.Entry(frame_1_TDES, font = "Courier 9", width=48)
        ...
        self.ruler.grid(row=6, column=1, columnspan=3, padx=5, pady=5, sticky=W)
        '''
        #   1.3.1 key length
        ...
        self.iv_label_TDES.grid(row=7, column=0, sticky=W)
        self.iv_textbox_TDES = tk.Entry(frame_1_TDES, font = "Courier 9", width=48)
        ...
        self.input_label_TDES.grid(row=8, column=0, sticky=W)
        self.input_textbox_TDES = tk.Entry(frame_1_TDES, font = "Courier 9", width=48)
        ...
        #   RULER !
        self.ruler = tk.Label(frame_1_TDES, text="|----8 Bytes---||----8 Bytes---||----8 Bytes---|", font="Courier 9",width=48)
        self.ruler.grid(row=9, column=1, columnspan=3, padx=5, pady=5, sticky=W)
        #   1.6 Output Data Entry Textbox
        self.output_label_TDES = tk.Label(frame_1_TDES, text="Output value: ")
        self.output_label_TDES.grid(row=10, column=0, sticky=W)
        self.output_textbox_TDES = tk.Text(frame_1_TDES, font = "Courier 9", height=8, width=48)
        ...
        self.key_textbox_AES = tk.Entry(frame_2_AES, font = "Courier 9", width=48)
        self.key_textbox_AES.grid(row=5, column=1, columnspan=3, padx=5, pady=5, sticky=W)
        #   RULER !
        self.ruler = tk.Label(frame_2_AES, text="|----8 Bytes---||----8 Bytes---||----8 Bytes---|", font="Courier 9", width=48)
        self.ruler.grid(row=6, column=1, columnspan=3, padx=5, pady=5, sticky=W)
        #   2.4 IV Entry Textbox
        self.iv_label_AES = tk.Label(frame_2_AES, text="IV value: ")
        self.iv_label_AES.grid(row=7, column=0, sticky=W)
        self.iv_textbox_AES = tk.Entry(frame_2_AES, font = "Courier 9", width=48)
        ...
        self.input_label_AES.grid(row=8, column=0, sticky=W)
        self.input_textbox_AES = tk.Entry(frame_2_AES, font = "Courier 9", width=48)
        ...
        self.output_label_AES.grid(row=10, column=0, sticky=W)
        self.output_textbox_AES = tk.Text(frame_2_AES, font = "Courier 9", height=8, width=48)
        ...
        self.RSA_pri_key_lb.grid(row=2, column=1, padx=5, pady=4, sticky=W+N)

        self.pri_key_textbox = tk.Text(self.Key_Gen, font = "Courier 9", height=8, width=64)
        self.pri_key_textbox.grid(row=2, column=2, padx=5, pady=5, sticky=W)

        self.RSA_pub_key_lb = tk.Label(self.Key_Gen, text="Public     ")
        self.RSA_pub_key_lb.grid(row=3, column=1, padx=5, pady=5, sticky=W+N)

        self.pub_key_textbox = tk.Text(self.Key_Gen, font = "Courier 9", height=8, width=64)
        self.pub_key_textbox.grid(row=3, column=2, padx=5, pady=5, sticky=W)

        self.RSA_key_pair_gen_bot = tk.Button(self.Key_Gen, text="Gen RSA key pair", width=18, command=self.RSA_key_pair_gen_func)
        self.RSA_key_pair_gen_bot.grid(row=4, column=2, padx=5, pady=5, sticky=E)

        self.RSA_k_len_lb = tk.Label(self.Key_Gen, text="key len")
        self.RSA_k_len_lb.grid(row=4, column=1, padx=5, pady=5, sticky=W)
        self.RSA_k_len_En = tk.Entry(self.Key_Gen, width=5)
        self.RSA_k_len_En.grid(row=4, column=2, padx=5, pady=5, sticky=W)
        ...
        self.rsa_data_in = tk.Text(frame_3_2_RSA, font = "Courier 9", height=8, width=56)
        self.rsa_data_in.grid(row=1, column=2, padx=5, pady=5, sticky=W)

        self.rsa_data_out_lb = tk.Label(frame_3_2_RSA, text="Data Output")
        self.rsa_data_out_lb.grid(row=2, column=1, sticky=W+N)
        self.rsa_data_out = tk.Text(frame_3_2_RSA, font = "Courier 9", height=8, width=56)
        self.rsa_data_out.grid(row=2, column=2, padx=5, pady=5, sticky=W)


        #   3_2.1 RSA Enc/Dec
        self.key_pub_butt_RSA = tk.Button(frame_3_2_RSA, text="Encrypt data", width=11, command=self.execution_RSA_enc)
        self.key_pub_butt_RSA.grid(row=3, column=2, padx=5, pady=5, sticky=E)
        self.key_pri_butt_RSA = tk.Button(frame_3_2_RSA, text="Decrypt data", width=11, command=self.execution_RSA_dec)
        self.key_pri_butt_RSA.grid(row=4, column=2, padx=5, pady=5, sticky=E)
        '''
        ...
        self.hash_hmac_key_entry = tk.Entry(frame_4_HASH, font = "Courier 9", width=48)
        ...
        self.hash_input_entry = tk.Entry(frame_4_HASH, font = "Courier 9", width=48)
        ...
        self.hash_output_text = tk.Text(frame_4_HASH, font = "Courier 9", height=8, width=48)
        ...
        self.xor_inputA_value = tk.Entry(self.XOR_frame, font = "Courier 9", width=47)
        self.xor_inputA_value.grid(row=2, column=2, columnspan=2, padx=5, pady=5, sticky=W)
        self.xor_inputB_value = tk.Entry(self.XOR_frame, font = "Courier 9", width=47)
        self.xor_inputB_value.grid(row=3, column=2, columnspan=2, padx=5, pady=5, sticky=W)
        self.xor_result_value = tk.Text(self.XOR_frame, font = "Courier 9", height=8, width=47)
        ...
        self.rng_8B_textbox = tk.Text(self.rng_bar_bar, font = "Courier 9", height=1, width=48)
        ...
        self.rng_32B_textbox = tk.Text(self.rng_bar_bar, font = "Courier 9", height=4, width=48)
        ...
        self.rng_88B_textbox = tk.Text(self.rng_bar_bar, font = "Courier 9", height=4, width=48)
