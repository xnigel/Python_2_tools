...
#   20. CRC - CRC16/BUYpass calculator is added on 20171119...
#	21. dukpt - adding
...
root = tk.Tk()
root.title('CryptoBox v01.04.05 20171215')
root.geometry("540x550+200+200")    #("560x480+0+0") for Linux; ("530+470+20+20") for Windows
...
frame_8_CRC = ttk.Frame(algo_tab)
frame_9_dukpt = ttk.Frame(algo_tab)
frame_10_ABT = ttk.Frame(algo_tab)
...
algo_tab.add(frame_8_CRC, text='CRC\ncalc.')
algo_tab.add(frame_9_dukpt, text='DUKPT\n')
algo_tab.add(frame_10_ABT, text='About\n...')
algo_tab.pack()
...
default_bdk = StringVar(frame_9_dukpt, value = "0123456789ABCDEFFEDCBA9876543210")
default_ksn = StringVar(frame_9_dukpt, value = "FFFF9876543210E00000")
default_iv = "0000000000000000"
default_keymask = "C0C0C0C000000000C0C0C0C000000000"
...
class CryptoBox(tk.Tk):
    ...
    def execution_TDES(self):
        ...
        if selection_Algo == 0:  # non DES/TDES algo
            ...
            if selection_EorD == 0 or key_len != 8:
                ...
                h_out_data_e = output_raw_e.encode('hex').upper()
                ...
                h_out_data_d = output_raw_d.encode('hex').upper()
            ...
            print "TDES mode              :", mode, " | <1:ECB, 2:CBC, 3:CFB, 4:OFB>"
            #   'Enc/Dec' Judgment, and execute!!!
            if selection_EorD == 0:
                self.output_textbox_TDES.delete(1.0, END)
                ...
                output_raw_e = obj.encrypt(h_in_data_xDES)
                h_out_data_e = output_raw_e.encode('hex').upper()
                print "TDES output (Enc)      :", h_out_data_e
                # print h_out_data_e
                # print output_raw_e
                self.output_textbox_TDES.delete(1.0, END)
                self.output_textbox_TDES.insert(1.0, h_out_data_e)
    ...
    def execution_AES(self):
        ...
        print "initialization vector  :", hiv_AES.encode('hex')
        input_raw_AES = self.input_textbox_AES.get()
        h_in_data_AES = input_raw_AES.replace(' ', '').decode('hex')
        print "AES input data         :", h_in_data_AES.encode('hex').upper()
        ...
        print "AES mode is            :", mode, " | <1:ECB, 2:CBC, 3:CFB, 4:OFB>"
        #   'Enc/Dec' Judgment, and execute!!!
        if selection_aes_EorD == 0:
            ...
            h_out_data_aes_e = output_aes_raw_e.encode('hex').upper()
            ...
            h_out_data_aes_d = output_aes_raw_d.encode('hex').upper()
    ...
    def RSA_key_pair_gen_func(self):
        ...
        private = RSA.generate(int(rsa_len), e=3)
        ...
        with open("UL_CryptBox_RSA_pub.pem", "w") as pub_key:
            pub_key.write(public.exportKey())
        with open("UL_CryptBox_RSA_pvt.pem", "w") as pvt_key:
            pvt_key.write(private.exportKey())


        self.RSAgen_output_exp_tx.delete(1.0, END)
        self.RSAgen_output_exp_tx.insert(1.0, hex(private.e).rstrip("L").lstrip("0x").upper())
        self.RSAgen_output_pri_tx.delete(1.0, END)
        self.RSAgen_output_pri_tx.insert(1.0, hex(private.d).rstrip("L").lstrip("0x").upper())
        self.RSAgen_output_pub_tx.delete(1.0, END)
        self.RSAgen_output_pub_tx.insert(1.0, hex(private.n).rstrip("L").lstrip("0x").upper())

        self.rsa_key_exp_import_tx.delete(1.0, END)
        self.rsa_key_exp_import_tx.insert(1.0, hex(private.e).upper())
        self.rsa_key_pri_import_tx.delete(1.0, END)
        self.rsa_key_pri_import_tx.insert(1.0, hex(private.d).upper())
        self.rsa_key_pub_import_tx.delete(1.0, END)
        self.rsa_key_pub_import_tx.insert(1.0, hex(private.n).upper())

        with open("UL_CryptBox_rsa_e_imported.imp", "w") as RSA_e:
            RSA_e.write(hex(private.e))
        with open("UL_CryptBox_rsa_d_imported.imp", "w") as RSA_d:
            RSA_d.write(hex(private.d))
        with open("UL_CryptBox_rsa_n_imported.imp", "w") as RSA_n:
    ...
    def rsa_key_import_func(self):
        ...
        if import_flag == 0:
            ...
            with open('UL_CryptBox_rsa_e_imported.imp', 'r') as RSA_e_temp:
                test_value_e = RSA_e_temp.read()
            with open('UL_CryptBox_rsa_d_imported.imp', 'r') as RSA_d_temp:
                test_value_d = RSA_d_temp.read()
            with open('UL_CryptBox_rsa_n_imported.imp', 'r') as RSA_n_temp:
                test_value_n = RSA_n_temp.read()
            
            print "\n==================================================================================="
            print "RSA keys are imported from the *.imp files!! --------------------------------------\n"
            print "test_value_e:\n", long(test_value_e, 16)
            print "\ntest_value_d:\n", long(test_value_d, 16)
            print "\ntest_value_n:\n", long(test_value_n, 16)
            #print "\nrsa_exp_import:\n", rsa_exp_import
            print "RSA keys are imported from the *.imp files!! ----------------------------------END-\n"
            ...
            with open("UL_CryptBox_rsa_e_imported.imp", "w") as RSA_e_imported:
                RSA_e_imported.write(rsa_exp_import)
            with open("UL_CryptBox_rsa_d_imported.imp", "w") as RSA_d_imported:
                RSA_d_imported.write(rsa_pri_import)
            with open("UL_CryptBox_rsa_n_imported.imp", "w") as RSA_n_imported:
            ...
            print "\n==================================================================================="
            print "RSA keys are imported from the 3 textboxes!! --------------------------------------\n"
            print "test_value_e:\n", long(test_value_e, 16)
            print "\ntest_value_d:\n", long(test_value_d, 16)
            print "\ntest_value_n:\n", long(test_value_n, 16)
            print "\nrsa_exp_import:\n", rsa_exp_import
            print "RSA keys are imported from the 3 textboxes!! ----------------------------------END-\n"
    ...
    def execution_RSA_enc(self):
        ...
        self.rsa_data_out.insert(1.0, hex(ciphertext_output).upper())
    ...
    def execution_RSA_dec(self):
        ...
        self.rsa_data_out.delete(1.0, END)
        self.rsa_data_out.insert(1.0, hex(plaintext_output).upper())
    ...
    def execution_HASH(self):
        ...
        if hash_algo_selector_hmac ==0:
            ...
            if hash_algo_selector == 0:
                ...
                hash_algo = "digestmod=SHA"
            elif hash_algo_selector ==2:
                hash_algo = "digestmod=MD4"
            elif hash_algo_selector ==3:
                hash_algo = "digestmod=MD5"
            elif hash_algo_selector ==5:
                hash_algo = "digestmod=SHA224"
            elif hash_algo_selector ==6:
                hash_algo = "digestmod=SHA256"
            elif hash_algo_selector ==7:
                hash_algo = "digestmod=SHA384"
            elif hash_algo_selector ==8:
                hash_algo = "digestmod=SHA512"
            else:
                pass
    ...
    def execution_CRC(self):
        self.CRC_calc_output_value.delete("1.0", END)
        CRC_input_string = self.CRC_calc_input_value.get()
        print CRC_input_string
        CRC_input_hex = binascii.b2a_hex(CRC_input_string)
        print CRC_input_hex
        crc16 = crcmod.mkCrcFun(0x18005, rev=False, initCrc=0x0000, xorOut=0x0000)
        print crc16
        CRC_output = hex(crc16(CRC_input_hex.decode("hex")))
        print CRC_output
        self.CRC_calc_output_value.insert(1.0, CRC_output.upper())

    def execution_dukpt(self):
        self.dukpt_iv_tx.delete("1.0", END)
        self.dukpt_keymask_tx.delete("1.0", END)
        self.dukpt_iv_tx.insert(1.0, default_iv)
        self.dukpt_keymask_tx.insert(1.0, default_keymask)

        ksn = self.dukpt_ksn_tx.get()
        shifted_ksn = ksn [:16]
        self.dukpt_ksnshift_tx.delete("1.0", END)
        self.dukpt_ksnshift_tx.insert(1.0, shifted_ksn)
        print "ksn:", ksn, "\n", "shifted_ksn", shifted_ksn

        #xor: bdk and keymask
        data_bdk = self.dukpt_bdk_tx.get()
        data_keymask = self.dukpt_keymask_tx.get("1.0", END).replace('\n', '')
        
        print data_bdk, len(data_bdk)
        print data_keymask, len(data_keymask)


        if len(data_bdk) != len(data_keymask):
            print 'String lengths must be equal'

        hstr_bdk = data_bdk.decode('hex')
        hstr_keymask = data_keymask.decode('hex')
        print "data_bdk", data_bdk

        out_bdkxor = ''
        for offset in range(0, len(hstr_bdk)):
            valA = int(hstr_bdk[offset].encode('hex'), 16)
            valB = int(hstr_keymask[offset].encode('hex'), 16)
            out_bdkxor += chr(valA ^ valB).encode('hex')
        bdkxor = out_bdkxor.upper()
        
        self.dukpt_bdkxor_tx.delete("1.0", END)
        self.dukpt_bdkxor_tx.insert(1.0, bdkxor)
        print "bdkxor           :", bdkxor

        ipek_iv = default_iv.replace(' ','').decode('hex')
        obj = DES3.new(hstr_bdk, DES3.MODE_ECB, ipek_iv)
        dec_shifted_ksn = shifted_ksn.decode('hex')
        ipek_L = obj.encrypt(dec_shifted_ksn)
        ipek_L_output = ipek_L.encode('hex').upper()
        print "hstr_bdk         :", hstr_bdk.encode('hex').upper()
        print "dec_shifted_ksn  :", dec_shifted_ksn.encode('hex').upper()
        print "ipek_iv          :", ipek_iv.encode('hex').upper()
        print "ipek_L           :", ipek_L.encode('hex').upper()
                
        obj = DES3.new(bdkxor.decode('hex'), DES3.MODE_ECB, ipek_iv)
        dec_shifted_ksn = shifted_ksn.decode('hex')
        ipek_R = obj.encrypt(dec_shifted_ksn)
        ipek_R_output = ipek_R.encode('hex').upper()
        print "ipek_R           :", ipek_R.encode('hex').upper()

        keymasksmall = default_keymask[:16]
        print "keymasksmall     :", keymasksmall
        self.dukpt_keymask_sm_tx.delete("1.0", END)
        self.dukpt_keymask_sm_tx.insert(1.0, keymasksmall)

        data_keymasksmall = keymasksmall.decode('hex')

        out_keyMaskedL = ''
        #ipek_L_dec = ipek_L.replace(' ', '').replace('\n', '').decode('hex')
        for offset in range(0, len(ipek_L)):
            valA_ipekL = int(ipek_L[offset].encode('hex'), 16)
            valB_keyMs = int(data_keymasksmall[offset].encode('hex'), 16)
            out_keyMaskedL += chr(valA_ipekL ^ valB_keyMs).encode('hex')
        keyMaskedL = out_keyMaskedL.upper()        
        
        out_keyMaskedR = ''
        #ipek_R_dec = ipek_R.replace(' ', '').replace('\n', '').decode('hex')
        for offset in range(0, len(ipek_R)):
            valA_ipekR = int(ipek_R[offset].encode('hex'), 16)
            valB_keyMs = int(data_keymasksmall[offset].encode('hex'), 16)
            out_keyMaskedR += chr(valA_ipekR ^ valB_keyMs).encode('hex')
        keyMaskedR = out_keyMaskedR.upper()
        print "keyMaskedL       :", keyMaskedL
        print "keyMaskedR       :", keyMaskedR
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ruler.grid(row=6, column=1, columnspan=3, padx=4, sticky=W)
        ...
        #   RULER !
        self.ruler = tk.Label(frame_1_TDES, text="|----8 Bytes---||----8 Bytes---||----8 Bytes---|", font="Courier 9",width=48)
        self.ruler.grid(row=9, column=1, columnspan=3, padx=4, sticky=W)
        ...
        self.output_textbox_TDES = tk.Text(frame_1_TDES, font = "Courier 9", height=2, width=64)
        ...
        self.output_textbox_AES = tk.Text(frame_2_AES, font = "Courier 9", height=2, width=64)
        ...
        self.RSAgen_pri_key_tx = tk.Text(self.Key_Gen, font = "Courier 9", height=3, width=64)
        ...
        self.RSAgen_pub_key_tx = tk.Text(self.Key_Gen, font = "Courier 9", height=3, width=64)
        ...
        self.RSAgen_output_pri_tx = tk.Text(self.Key_Gen, font = "Courier 9", height=3, width=64)
        ...
        self.RSAgen_output_pub_tx = tk.Text(self.Key_Gen, font = "Courier 9", height=3, width=64)
        ...
        self.ruler.grid(row=7, column=1, columnspan=3, padx=4, sticky=W)
        ...
        self.hash_output_text = tk.Text(frame_4_HASH, font = "Courier 9", height=4, width=64)
        ...
        self.ruler.grid(row=3, column=2, padx=4, sticky=W)
        ...
        self.xor_result_value = tk.Text(self.XOR_frame, font = "Courier 9", height=4, width=64)
        ...
        #   7   120 PINs
        self.pin_120_bar = tk.LabelFrame(frame_7_120, text=" Elapsed time of 120 online PIN entry log file generator ", font=("Helvetica", 12, "bold"), padx=5, pady=5)
        ...
        #   8   CRC calculator
        self.CRC_frame = tk.LabelFrame(frame_8_CRC, text = "CRC16 / BUYPASS", font=("Helvetica", 12, "bold"), padx=5, pady=5)
        self.CRC_frame.grid(row=1, column=1, rowspan=4, columnspan=4, sticky=NS)
        self.CRC_calc_input_lable = tk.Label(self.CRC_frame, text="Input :")
        self.CRC_calc_input_lable.grid(row=2, column=1, padx=5, sticky=W)
        self.CRC_calc_output_lable = tk.Label(self.CRC_frame, text="Output :")
        self.CRC_calc_output_lable.grid(row=3, column=1, padx=5, sticky=W)

        self.CRC_calc_input_value = tk.Entry(self.CRC_frame, font = "Courier 9", width=64)
        self.CRC_calc_input_value.grid(row=2, column=2, columnspan=2, padx=5, pady=5, sticky=W)
        self.CRC_calc_output_value = tk.Text(self.CRC_frame, font = "Courier 9", height = 1, width=64)
        self.CRC_calc_output_value.grid(row=3, column=2, columnspan=2, padx=5, pady=5, sticky=W)

        self.go_button_CRC = tk.Button(self.CRC_frame, text="Go!", width=10, command=self.execution_CRC)
        self.go_button_CRC.grid(row=4, column=3, padx=5, pady=5, sticky=E)

        
        #   9   DUKPT calculator
        self.dukpt_frame = tk.LabelFrame(frame_9_dukpt, text = "BDK / ksn", font=("Helvetica", 12, "bold"), padx=5, pady=5)
        self.dukpt_frame.grid(row=1, column=1, rowspan=4, columnspan=4, sticky=NS)

        self.dukpt_bdk_lable = tk.Label(self.dukpt_frame, text="BDK:")
        self.dukpt_bdk_lable.grid(row=2, column=0, sticky=E)
        self.dukpt_bdk_tx = tk.Entry(self.dukpt_frame, textvariable = default_bdk, font = "Courier 9", width=48)
        self.dukpt_bdk_tx.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky=W)
        self.dukpt_bdk_tx.configure(bg="yellow")
        #   RULER !
        self.dukpt_ruler = tk.Label(self.dukpt_frame, text="|----8 Bytes---||----8 Bytes---||----8 Bytes---|", font="Courier 9",width=48)
        self.dukpt_ruler.grid(row=3, column=1, columnspan=3, padx=4, sticky=W)
        
        self.dukpt_ksn_lable = tk.Label(self.dukpt_frame, text="ksn:")
        self.dukpt_ksn_lable.grid(row=4, column=0, sticky=E)
        self.dukpt_ksn_tx = tk.Entry(self.dukpt_frame, textvariable = default_ksn, font="Courier 9", width=48)
        self.dukpt_ksn_tx.grid(row=4, column=1, columnspan=3, padx=5, pady=5, sticky=W)
        self.dukpt_ksn_tx.configure(bg="yellow")
        self.dukpt_ksn_xx_lable = tk.Label(self.dukpt_frame, text="ksn_xx:")
        self.dukpt_ksn_xx_lable.grid(row=5, column=0, sticky=E)
        self.dukpt_ksn_xx_tx = tk.Entry(self.dukpt_frame, font="Courier 9", width=48)
        self.dukpt_ksn_xx_tx.grid(row=5, column=1, columnspan=3, padx=5, pady=5, sticky=W)        

        self.dukpt_ksnshift_lable = tk.Label(frame_9_dukpt, text="ksnshift:")
        self.dukpt_ksnshift_lable.grid(row=6, column=1, sticky=E)
        self.dukpt_ksnshift_tx = tk.Text(frame_9_dukpt, font="Courier 9", height=1, width=48)
        self.dukpt_ksnshift_tx.grid(row=6, column=2, columnspan=3, padx=5, pady=5, sticky=W)

        self.dukpt_keymask_lable = tk.Label(frame_9_dukpt, text="keymask:")
        self.dukpt_keymask_lable.grid(row=7, column=1, sticky=E)
        self.dukpt_keymask_tx = tk.Text(frame_9_dukpt, font="Courier 9", height=1, width=48)
        self.dukpt_keymask_tx.grid(row=7, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_iv_lable = tk.Label(frame_9_dukpt, text="iv:")
        self.dukpt_iv_lable.grid(row=8, column=1, sticky=E)
        self.dukpt_iv_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_iv_tx.grid(row=8, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_bdkxor_lable = tk.Label(frame_9_dukpt, text="bdkxor:")
        self.dukpt_bdkxor_lable.grid(row=9, column=1, sticky=E)
        self.dukpt_bdkxor_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_bdkxor_tx.grid(row=9, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_keymask_sm_lable = tk.Label(frame_9_dukpt, text="keymasksmall:")
        self.dukpt_keymask_sm_lable.grid(row=10, column=1, sticky=E)
        self.dukpt_keymask_sm_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_keymask_sm_tx.grid(row=10, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_keyMaskedL_lable = tk.Label(frame_9_dukpt, text="keyMasked-L:")
        self.dukpt_keyMaskedL_lable.grid(row=11, column=1, sticky=E)
        self.dukpt_keyMaskedL_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_keyMaskedL_tx.grid(row=11, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_keyMaskedR_lable = tk.Label(frame_9_dukpt, text="keyMasked-R:")
        self.dukpt_keyMaskedR_lable.grid(row=12, column=1, sticky=E)
        self.dukpt_keyMaskedR_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_keyMaskedR_tx.grid(row=12, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_returnKeyL_lable = tk.Label(frame_9_dukpt, text="returnKey-L:")
        self.dukpt_returnKeyL_lable.grid(row=13, column=1, sticky=E)
        self.dukpt_returnKeyL_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_returnKeyL_tx.grid(row=13, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_returnKeyR_lable = tk.Label(frame_9_dukpt, text="returnKey-R:")
        self.dukpt_returnKeyR_lable.grid(row=14, column=1, sticky=E)
        self.dukpt_returnKeyR_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_returnKeyR_tx.grid(row=14, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_returnKey_lable = tk.Label(frame_9_dukpt, text="returnKey:")
        self.dukpt_returnKey_lable.grid(row=15, column=1, sticky=E)
        self.dukpt_returnKey_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_returnKey_tx.grid(row=15, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.dukpt_SessionKey_lable = tk.Label(frame_9_dukpt, text="SessionKey:")
        self.dukpt_SessionKey_lable.grid(row=16, column=1, sticky=E)
        self.dukpt_SessionKey_tx = tk.Text(frame_9_dukpt, font="Courier 9",  height=1, width=48)
        self.dukpt_SessionKey_tx.grid(row=16, column=2, columnspan=3, padx=5, pady=3, sticky=W)

        self.go_button_dukpt = tk.Button(frame_9_dukpt, text="Go!", width=10, command=self.execution_dukpt)
        self.go_button_dukpt.grid(row=18, column=4, padx=5, pady=5, sticky=E)


        #    10   About CryptoBox
        self.abt_bar = tk.LabelFrame(frame_10_ABT, text=" --- CryptoBox --- ", font=("Helvetica", 12, "bold"))
...
update_timeText()
app = CryptoBox()
root.iconbitmap(r'C:\Python27\UL_logo_64.ico')
root.mainloop()
